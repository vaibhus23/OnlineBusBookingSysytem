...
public class ApplicationConfiguration extends WebMvcConfigurationSupport {
	...
	public Docket version1() {
		return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("com.org"))
				.paths(regex(".*" + ".*")).build().enable(true).groupName("OnlineBusBookingSystem")
				.apiInfo(new ApiInfoBuilder().description("OnlineBusBookingSystem").title("OnlineBusBookingSystem")
						.build());
	}
